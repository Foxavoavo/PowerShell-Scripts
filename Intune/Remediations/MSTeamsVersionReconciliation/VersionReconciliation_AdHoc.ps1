Function Get-TeamsObjects {
    ...
    If ($Usage -eq 'App') {
        Switch ($Version) {
            ...
            { [Version]$_ -lt [Version]$RepoAvailableVersion } {
                ...
                Write-Output 'Local Teams updated'
            }
    ...
    If ($Usage -eq 'User') {
        Switch ($Version) {
            { $Null -eq $_ } {}
            { [Version]$_ -lt [Version]$RepoAvailableVersion } {
                Write-Output "Local Teams needs update for $Username"
                ...
                Switch (Test-Path $uninstaller) {
                    $True {
                        ...
                        Write-Output "Uninstall Users teams from $Username"                               
                    }
                    $False {}
                }   
            }
            { [Version]$_ -ge [Version]$RepoAvailableVersion } {
                Write-Output "Local Teams will be removed from $Username profile"
                $uninstallerPath = $TeamsUserPath -replace ('\\\w*current\\\w*Teams.exe', '')
                $uninstaller = "$uninstallerPath\Update.exe"
                Switch (Test-Path $uninstaller) {
                    $True {
...
#get user sessions
$RAWSessions = Get-Process -IncludeUserName | select Username -Unique  | Select-String -Pattern '' ### Here add your AD DS domain name
